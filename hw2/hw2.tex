...
\begin{solution}
	\begin{equation}\begin{aligned}
		CFG_{usable} = 
			& \{\langle G, A\rangle | G = (\mathcal{N}, \Sigma, \mathcal{P}, \mathcal{S}) \texttt{ is a } \CFG, A \in \mathcal{P} \\
...
\begin{solution}
	\begin{equation}\begin{aligned}
		TM_{BLANK} = 
			& \{ \langle M\rangle \mid \exists w \in \Sigma^* \texttt{ when running } M \texttt{ on input } w \\
			& \texttt{ a blank symbol is writen over a nonblank }\}
	\end{aligned}\end{equation}
	\begin{proof}
